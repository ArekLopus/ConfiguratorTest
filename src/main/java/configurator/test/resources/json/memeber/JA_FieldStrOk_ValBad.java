...
// curl -i http://localhost:8080/ConfiguratorTest/res/config/jsonArrayFieldStrNameOkValueBad

// Field class member String name ok
@Path("/config")
public class JA_FieldStrOk_ValBad {
	
	String jas = "[\"arrStrFieldVal1\", \"arrStrFieldVal2\"";
	
	@Inject
	@ConfigJson(classMember = "jas")
	private TypedProperty<JsonArray> test;
	
	
	@Path("/jsonArrayFieldStrNameOkValueBad")
	@GET
	public Response testGet() {
		return Response.ok("Field class member String name ok: " + test).build();
	}
	
	@Path("/jsonArrayFieldStrNameOkValueBad/test")
	@GET
