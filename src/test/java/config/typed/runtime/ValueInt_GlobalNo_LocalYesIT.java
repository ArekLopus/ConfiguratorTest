...
public class ValueInt_GlobalNo_LocalYesIT {
	...
	public void typed_Integer_runtimeCheckFalse_AnnotationRuntimeCheckNo() {
...
//		WebTarget setPaths = client.target("http://localhost:8080/ConfiguratorTest/res/config/setPaths");
		...
		WebTarget runtimeCheck = client.target("http://localhost:8080/ConfiguratorTest/res/config/typedIntegerGlobalRuntimeNoLocalRuntimeYes/test");
		...
		TypedProperty<Integer> entity = requestForFirstTimeResponse.readEntity(new GenericType<TypedProperty<Integer>>(){});
		...
		// after change check
		entity = checkAfterChangeResponse.readEntity(new GenericType<TypedProperty<Integer>>(){});
		assertNotNull(entity.getValue());
		assertThat(entity.getValue(), instanceOf(Integer.class));
		assertThat(entity.getPropertyName(), is("testFilePropInt"));
		assertThat(entity.getValue(), is(151));
