...
public class DefValDouble_GlobalYes_LocalUseGlIT {
	...
	@Test
	// If runtime true and local uses global, always reload. If requested target changed we can see the change in the next request.
	public void typed_Double_DefVal_runtimeCheckTrue_AnnotationRuntimeCheckUseGlobal() {
		...
		WebTarget runtimeCheck = client.target("http://localhost:8080/ConfiguratorTest/res/config/typedDoubleDefValGlobalRuntimeYesLocalRuntimeUseGl/test");
		...
		assertNotNull(entity.getDefaultValue());
		assertThat(entity.getDefaultValue(), instanceOf(Double.class));
		assertThat(entity.getDefaultValue(), is(2.5));
