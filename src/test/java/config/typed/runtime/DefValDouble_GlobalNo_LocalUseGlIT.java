...
public class DefValDouble_GlobalNo_LocalUseGlIT {
	...
	@Test
	// If runtime false and local uses global, always from properties. Even if requested target changed we cant see the change in the next request. 
	public void typed_Double_DefVal_runtimeCheckFalse_AnnotationRuntimeCheckUseGlobal() {
...
//		WebTarget setPaths = client.target("http://localhost:8080/ConfiguratorTest/res/config/setPaths");
		...
		WebTarget runtimeCheck = client.target("http://localhost:8080/ConfiguratorTest/res/config/typedDoubleDefValGlobalRuntimeNoLocalRuntimeUseGl/test");
