...
public class DefValDouble_GlobalNo_LocalYesIT {
	...
	@Test
	// If runtime false and local is yes, always reload. If requested target changed we can see the change in the next request.
	public void typed_Double_DefVal_runtimeCheckFalse_AnnotationRuntimeCheckNo() {
...
//		WebTarget setPaths = client.target("http://localhost:8080/ConfiguratorTest/res/config/setPaths");
		...
		WebTarget runtimeCheck = client.target("http://localhost:8080/ConfiguratorTest/res/config/typedDoubleDefValGlobalRuntimeNoLocalRuntimeYes/test");
		...
		TypedProperty<Double> entity = requestForFirstTimeResponse.readEntity(new GenericType<TypedProperty<Double>>(){});
		...
		assertThat(entity.getDefaultValue(), is(3.67));
		
		// after change check
		entity = checkAfterChangeResponse.readEntity(new GenericType<TypedProperty<Double>>(){});
		assertNotNull(entity.getDefaultValue());
		assertThat(entity.getDefaultValue(), instanceOf(Double.class));
		assertThat(entity.getDefaultValue(), is(2.50));
