...
public class Bool_NameBadIT {
	...
	public void typed_Boolean_NameBad() {
		
		WebTarget target = client.target("http://localhost:8080/ConfiguratorTest/res/config/typedParameterBooleanNameBad/test");
		
		Response response = target.request(MediaType.APPLICATION_JSON).get();
		TypedProperty<Boolean> entity = response.readEntity(new GenericType<TypedProperty<Boolean>>(){});
		
		assertNotNull(entity);
		assertThat(entity.getPropertyName(), is("BooleanBadPropertyName"));
